...
export default function drawPolylines({
  ...
  const polylines = bufferStore.polylines;
  ...
  polylines.forEach((polyline) => {
    const points = polyline.data.subarray(0, -5);
    const color = polyline.data.subarray(points.length, -1);
    const thickness = polyline.data.at(-1) ?? 1;
